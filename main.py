...
encoder1 = Encoder(int(config["hidden_size_encoder"]),len(source_vocab)+1,int(config["batch_size"]), num_layers=int(config["num_layer_encoder"]),bidirectional=bidirectional).to(device)
decoder1 = Decoder(int(config["hidden_size_decoder"]),len(target_vocab)+1, int(config["batch_size"]), num_layers=int(config["num_layer_decoder"])).to(device)
trainIters(trainset,encoder1, decoder1,num_epochs=int(config["num_epoch"]),batch_size=int(config["batch_size"]),print_every=10,device=device)
